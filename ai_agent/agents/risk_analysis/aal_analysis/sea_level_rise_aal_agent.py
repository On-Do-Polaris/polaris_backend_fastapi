'''
파일명: sea_level_rise_aal_agent.py
최종 수정일: 2025-11-20
버전: v9
...
class SeaLevelRiseAALAgent(BaseAALAnalysisAgent):
	"""
	...
	def calculate_intensity_indicator(self, collected_data: Dict[str, Any]) -> np.ndarray:
		...
		Args:
			collected_data: 수집된 해양 및 지형 데이터
				- zos_data: 연도별 시점별 zos 데이터
					각 원소는 {'year': int, 'zos_values': [cm values]}
		...
		ground_level = ocean_data.get('ground_level', 5.0)  # 기본값 5m
		...
		for year_data in zos_data:
			...
			zos_values = year_data.get('zos_values', [])  # cm 단위
			...
			# 시점별 침수 깊이 계산
			inundation_depths = []
			for zos_cm in zos_values:
				zos_m = zos_cm / 100.0  # cm → m 변환
