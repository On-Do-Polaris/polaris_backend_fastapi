...
class BuildingCharacteristicsAgent:
    ...
    def _analyze_structural_features(self, building_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _analyze_location_features(self, building_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def _calculate_structural_grade(self, building_info: Dict[str, Any]) -> str:
        """구조적 등급 계산"""
        score = 0
    ...
    def _assess_water_proximity(self, location: Dict[str, Any]) -> str:
        """수계 근접성 평가 (향후 GIS 데이터 활용 가능)"""
        # 현재는 간단한 평가, 향후 GIS 데이터 연동 필요
        return 'Unknown'

    def _interpret_single_risk(self, hazard_type: str, score: float, level: str) -> Dict[str, Any]:
        """개별 리스크 해석"""
        interpretation = {
    ...
    def _interpret_integrated_risk(self, integrated_risk: Dict[str, Any]) -> str:
        """통합 리스크 해석"""
    ...
    def _generate_fallback_analysis(
        ...
        vulnerability_factors: List[Dict[str, Any]],
        resilience_factors: List[Dict[str, Any]],
    ...
    def _llm_comprehensive_analysis(
        ...
        vulnerability_factors: List[Dict[str, Any]],
        resilience_factors: List[Dict[str, Any]],
        additional_data_guideline: Optional[Dict[str, Any]] = None
    ) -> str:
        """LLM 기반 종합 분석"""
        ...
        prompt = (
            ...
            "## 건물 기본 정보\n"
            f"- 건물 연식: {building_info.get('building_age', 0)}년\n"
            f"- 구조: {building_info.get('structure', 'Unknown')}\n"
            f"- 위치: {location_features.get('address', 'Unknown')}\n\n"
            "## 구조적 특징\n"
            f"{structural_features}\n\n"
            "## 리스크 분석 결과\n"
            f"{risk_interpretation}\n\n"
            "## 취약 요인\n"
            f"{vulnerability_factors}\n\n"
            "## 회복력 요인\n"
            f"{resilience_factors}\n\n"
        )
        ...
        if apply_guideline and suggested_insights:
            prompt += (
                "\n## 추가 고려사항 (사용자 제공 데이터)\n"
                f"관련도: {relevance:.2f}\n"
                f"{chr(10).join([f'- {insight}' for insight in suggested_insights])}\n"
            )
