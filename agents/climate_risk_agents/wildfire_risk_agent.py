'''
파일명: wildfire_risk_agent.py
최종 수정일: 2025-11-05
버전: v00
파일 개요: 산불 리스크 분석 에이전트 (화재 위험 기상 조건, 식생 건조도, 산불 빈도)
'''
...
class WildfireRiskAgent(BaseClimateRiskAgent):
	...
	def calculate_hazard(self, data: Dict, ssp_weights: Dict[str, float]) -> float:
		"""
		산불 Hazard 계산
		SSP 시나리오별 기온/강수 변화 및 산불 기상 지수 분석
